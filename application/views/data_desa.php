...
<body>
                              ...
                              <table id="zero_config" class="responsive-table display" style="width:100%">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                              <a class="waves-effect waves-light btn  orange btn tooltipped"data-position="bottom" data-delay="50" data-tooltip="Edit" onclick="edit('<?php echo $data_desa->kode_desa;?>')"><i class="material-icons dp48">edit</i></a>
                                              <?php
            ...
            <div id="modal" class="modal">
                <div class="modal-content">
                    ...
                    <div class="row">
                        <div class="input-field col s12">
                            <i class="material-icons prefix">chrome_reader_mode</i>
                            <input  type="text"  required name="id_desa_edit" id="id_desa_edit" autofocus>
                            <div class="errorTxt2"></div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="input-field col s12">
                            <i class="material-icons prefix">chrome_reader_mode</i>
                            <input  type="text"  required name="nama_desa_edit" id="nama_desa_edit" >
                            <input  type="hidden"  required name="kode_desa" id="kode_desa">
                            <div class="errorTxt2"></div>
                        </div>
                    </div>
    ...
    function edit(kode_desa){
      $.ajax({
          type : "POST",
          url  : "<?php echo base_url('provinsi/edit_desa')?>/"+kode_desa,
          dataType : "JSON",
    ...
    function data_modal(data){
          $('[name="nama_desa_edit"]').val(data[0]["nama_desa"]);
          $('[name="id_desa_edit"]').val(data[0]["id_desa"]);
          $('[name="kode_desa"]').val(data[0]["kode_desa"]);
          $('#modal').modal('open');
    }


    $('#simpan').on('click',function(){
        var nama_desa_edit=$('#nama_desa_edit').val();
        var id_desa_edit=$('#id_desa_edit').val();
        var kode_desa=$('#kode_desa').val();
        $.ajax({
        type : "POST",
        url  : "<?php echo base_url()?>provinsi/simpan_edit_desa/"+kode_desa+"/"+id_desa_edit+"/"+nama_desa_edit,
        dataType : "JSON",
                success: function(notif){
                    $('#modal').modal('close');
                    if (notif==1) {
                      berhasil("Data Berhasi Diperbaharui!.");
                      setTimeout("location.href = '<?php echo base_url()?>datadesa?kec=<?php echo $this->input->get('kec');?>';",1500);
                    }else{
