...
class Cetak extends CI_Controller {
	...
	public function cetak_bt_desa_a3(){
		require_once(APPPATH . 'third_party/fpdf/fpdf.php');
		error_reporting(E_ALL);
		$kode_desa=$this->uri->segment('3');
		$jumlah_bt=$this->uri->segment('4');
		...
		if($filecount==$jumlah_bt){
				...
				foreach ($data1 as $isi1) {
					$a[$i]=$isi1->id_buku_tanah;
					$b[$i]=$isi1->id_jenis_hak.".".$isi1->no_hak;
					$i++;
					$nama_desa=$isi1->nama_desa;
				}
				$x=8;
				...
				$pdf->Text(10, 10,"Barcode Buku Tanah Desa ".$nama_desa);
				for($j=0; $j<$jumlah; $j++){
					$pdf->Image("qr/BT-".$kode_desa."/BT-".$a[$j].".png", $x, $y, $w, $h, "png");
					$pdf->Text($x+10, $y+34,$b[$j]);
					$x=$x+31.5;$baris++;$kertas++;
					if($baris==9){
						$x=8;
						$y=$y+36.5;
						$baris=0;
	...
	public function cetak_bundel(){
		...
		$jumlah_warkah;
			...
			$sql="SELECT * FROM `bundel` LEFT JOIN baris on baris.id_baris=bundel.id_baris left JOIN lemari on lemari.id_lemari=baris.id_lemari";
			$query = $this->db->query($sql);
			$data=$query->result();
			foreach ($data as $isi) {
				QRcode::png(
				"L-".$isi->id_lemari,
				"qr/Bndl/Bndl-".$isi->id_bundel.".png",
				$level = QR_ECLEVEL_H,
				$size  = 6,
				$margin = 2
			);
		...
		$filecount . "files ";
				...
				$pdf->SetFont('Times','',11);
				$pdf->AddPage();
				$sql1="SELECT * FROM `bundel` LEFT JOIN baris on baris.id_baris=bundel.id_baris left JOIN lemari on lemari.id_lemari=baris.id_lemari";
				$query1 = $this->db->query($sql1);
				$data1=$query1->result();
				$i=0;$a;$b;$j;$x=0;
				$jumlah=count($data1);
